...
int fill_superblock(struct vdfs4_sb_info *sbi)
{
	...
	if (sizeof(GIT_HASH) != 0)
		memcpy(sbi->sb.mkfs_git_hash, GIT_HASH,
				min(sizeof(sbi->sb.mkfs_git_hash), strlen(GIT_HASH)));
#endif
...
static void copy_cmd_line(int argc, char *argv[], char *buffer)
{
	...
	//Store user name
	if (getlogin_r( cmd_block->user_name, sizeof(cmd_block->user_name)))
		strncpy( cmd_block->user_name, "Anonymous", sizeof(cmd_block->user_name));
