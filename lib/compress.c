...
static int compress_chunk(unsigned char *in, int ilen,
		...
		int volume_chunk_size, int comp_ratio)
{
	int ret = 0;
	int have_compress_ratio = 0;
	int max_compress_ratio = -1;
	int compressed_size = -1;
	size_t align;

#ifdef COMPRESS_RATIO
	max_compress_ratio = atoi(COMPRESS_RATIO);
#endif
	if (max_compress_ratio < 0 || max_compress_ratio > 100)
		max_compress_ratio = 75;

	if(comp_ratio != -1)
		max_compress_ratio = comp_ratio;

	ret = compressor[compress_type](in, ilen, out, olen, &compressed_size);
	if(ret)
		return ret;
	have_compress_ratio = (compressed_size * 100) / ilen;

	/* someone setting 100 probably wishses to limit only bigger
	 * output than input but sometimes ratio precision is not enough */
	if(max_compress_ratio == 100 && compressed_size > ilen)
		goto chunk_uncomp;

	if(have_compress_ratio > max_compress_ratio)
		goto chunk_uncomp;
	else
		goto chunk_comp;
...
void compress_chunk_thread(void *arg)
{
	...
	while (!tinfo->exit) {
		...
		ret = compress_chunk(tinfo->in, tinfo->in_size,
				...
				tinfo->max_chunk_size, tinfo->compress_ratio);
		if (ret) {
			ret = -errno;
			goto err_exit;
		}
		...
		const struct profiled_file* pfile)
{
	...
	int compress_ratio = -1, no_written_chunks = 0;
	int force_compress = 0;
	...
	if(aes_info->do_encrypt &&
		IS_FLAG_SET(sbi->service_flags, FORCE_COMPR_ENCRYPTED))
		force_compress = 1;

	if(force_compress)
		/* In this case we must ensure that all chunks of
		 * file will be compressed. Setting high compression
		 * ratio will do the trick. */
		compress_ratio = 20000;
...
compress_retry:
	...
	*new_file_size = 0;
	...
	for (count = 1; count <= chunks; count++) {
		...
		thread[thread_num].compress_ratio = compress_ratio;
	...
	*ext = ext_table;
	...
	if(force_compress &&
			!is_all_chunks_compressed(ext_table, chunks)) {
		/* Not all chunks has been compressed despite very high
		 * compression ratio.
		 * Sometimes compressed output could be larger than input
		 * and would exceed volume_chunk_size which is forbidden
		 * due to hw reasons.
		 * Lets decrease log_chunk_size, this way compressed output
		 * will never exceed original volume_chunk_size. */
		if((unsigned)chunk_size == volume_chunk_size) {
			log_chunk_size -= 1;
...
void compress_file_thread(void *arg)
{
	...
	while (!tinfo->exit) {
		...
		if (IS_FLAG_SET(f_rec->common.flags, VDFS4_NOCOMPRESS_FILE)) {
			...
			/* ELF files must stay compressed to retain auth feature */
			if(!is_elf_file_path(tinfo->ptr->src_full_path)) {
				ret = 0;
...
do_compress:
		...
		if (ret) {
			if (ret == -ENOTCOMPR) {
				ret = 0;
				if(FORCE_COMPR_ENCRYPTED)
					assert(!(tinfo->ptr->cmd & CMD_ENCRYPT));
			} else {
